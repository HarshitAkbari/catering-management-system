    ...
    <div class="row">
        <div class="col-lg-12">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('roles.update', $role) }}" method="POST" class="needs-validation" novalidate>
                        @csrf
                        @method('PUT')
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control" value="{{ ucfirst($role->name) }}" readonly>
                            </div>
                            <div class="col-md-6">
                                <label for="display_name" class="form-label">Display Name</label>
                                <input type="text" name="display_name" id="display_name" class="form-control" value="{{ old('display_name', $role->display_name) }}" placeholder="e.g., Manager - Sales">
                                @error('display_name')
                        ...
                        <div class="row mb-3">
                            <div class="col-md-12">
                                ...
                                <textarea name="description" id="description" class="form-control" rows="2">{{ old('description', $role->description) }}</textarea>
                                @error('description')
                                    <div class="invalid-feedback d-block">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="row mb-3">
                            <div class="col-md-12">
                                ...
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="permission_type" id="permission_type_read" value="read" {{ old('permission_type', $role->permission_type) === 'read' ? 'checked' : '' }} required>
                                    <label class="form-check-label" for="permission_type_read">
                                        Read - User can only view data
                                    </label>
                                </div>
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="permission_type" id="permission_type_write" value="write" {{ old('permission_type', $role->permission_type) === 'write' ? 'checked' : '' }} required>
                        ...
                        <div class="row mb-3" id="write_permissions_section" style="display: none;">
                            <div class="col-md-12">
                                ...
                                @php
                                    $selectedWritePerms = old('write_permissions', $role->write_permissions ?? []);
                                @endphp
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" name="write_permissions[]" id="write_add" value="add" {{ in_array('add', $selectedWritePerms) ? 'checked' : '' }}>
                                    <label class="form-check-label" for="write_add">Add</label>
                                </div>
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" name="write_permissions[]" id="write_edit" value="edit" {{ in_array('edit', $selectedWritePerms) ? 'checked' : '' }}>
                                    <label class="form-check-label" for="write_edit">Edit</label>
                                </div>
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" name="write_permissions[]" id="write_delete" value="delete" {{ in_array('delete', $selectedWritePerms) ? 'checked' : '' }}>
                                    <label class="form-check-label" for="write_delete">Delete</label>
                                </div>
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" name="write_permissions[]" id="write_export" value="export" {{ in_array('export', $selectedWritePerms) ? 'checked' : '' }}>
                        ...
                        <div class="row mb-3">
                            <div class="col-md-12">
                                ...
                                <div class="border rounded p-3" style="max-height: 400px; overflow-y: auto;">
                                    ...
                                    @else
                                        @php
                                            $selectedMenuIds = old('menu_ids', $role->menus->pluck('id')->toArray());
                                        @endphp
                                        @foreach($parentMenus as $parentMenu)
                                        <div class="mb-3">
                                            <div class="form-check">
                                                <input class="form-check-input parent-menu" type="checkbox" name="menu_ids[]" id="menu_{{ $parentMenu->id }}" value="{{ $parentMenu->id }}" {{ in_array($parentMenu->id, $selectedMenuIds) ? 'checked' : '' }}>
                                            ...
                                            @if($childMenus->count() > 0)
                                                <div class="ms-4 mt-2">
                                                    @foreach($childMenus as $childMenu)
                                                        <div class="form-check">
                                                            <input class="form-check-input child-menu" type="checkbox" name="menu_ids[]" id="menu_{{ $childMenu->id }}" value="{{ $childMenu->id }}" data-parent="{{ $parentMenu->id }}" {{ in_array($childMenu->id, $selectedMenuIds) ? 'checked' : '' }}>
