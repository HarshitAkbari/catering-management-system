    ...
    <div class="dlabnav-scroll">
        <ul class="metismenu" id="menu">
            @if(isset($userMenus) && $userMenus->count() > 0)
                        ...
                        @php
                            ...
                            if($menu->children && $menu->children->count() > 0) {
                                foreach($menu->children as $child) {
                                    if($child->route && request()->routeIs(str_replace('.', '.*', $child->route))) {
                                        $hasActiveChild = true;
                                        break;
                                    }
                                }
                            }
                            $isActive = request()->routeIs(str_replace('.', '.*', $menu->name . '.*')) || $hasActiveChild;
                        @endphp
                        <li class="{{ $isActive ? 'mm-active' : '' }}">
                            <a class="has-arrow {{ $isActive ? 'mm-active' : '' }}" href="javascript:void(0);" aria-expanded="{{ $isActive ? 'true' : 'false' }}">
                                @if($menu->icon)
                                    <i class="{{ $menu->icon }}"></i>
                                @endif
                                <span class="nav-text">{{ $menu->display_name }}</span>
                ...
                @endforeach
            @else
                {{-- Fallback to default menu if no menus are configured or seeded --}}
                @if(auth()->check() && auth()->user()->isAdmin())
                    {{-- Show default menu structure for admin if menus not seeded --}}
                    ...
                    <li class="{{ request()->routeIs('settings.*') || request()->routeIs('users.*') ? 'mm-active' : '' }}"><a class="has-arrow {{ request()->routeIs('settings.*') || request()->routeIs('users.*') ? 'mm-active' : '' }}" href="javascript:void(0);" aria-expanded="{{ request()->routeIs('settings.*') || request()->routeIs('users.*') ? 'true' : 'false' }}">
                        ...
                        <ul class="{{ request()->routeIs('settings.*') || request()->routeIs('users.*') ? 'mm-show' : '' }}" aria-expanded="{{ request()->routeIs('settings.*') || request()->routeIs('users.*') ? 'true' : 'false' }}">
                            ...
                            <li><a href="{{ route('roles.index') }}" class="{{ request()->routeIs('roles.*') ? 'mm-active' : '' }}">Roles & Permissions</a></li>
